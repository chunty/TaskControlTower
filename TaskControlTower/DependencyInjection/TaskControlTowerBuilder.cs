...
public sealed class TaskControlTowerBuilder(IServiceCollection services)
{
    ...
    /// Task keys are prefixed with <see cref="TaskControlTowerOptions.KeyPrefix"/> (default "cm:") to avoid collisions.
    /// </summary>
    public TaskControlTowerBuilder AddDistributedStore()
    {
        ReplaceStore(services => services.AddSingleton<ITaskStateStore>(sp =>
        {
            var opts = sp.GetRequiredService<TaskControlTowerOptions>();
    ...
    /// Task keys are prefixed with <see cref="TaskControlTowerOptions.KeyPrefix"/> (default "cm:").
    /// </summary>
    public TaskControlTowerBuilder AddDistributedStore(Func<IServiceProvider, IDistributedCache> cacheFactory)
    {
        ReplaceStore(services => services.AddSingleton<ITaskStateStore>(sp =>
        {
            var opts = sp.GetRequiredService<TaskControlTowerOptions>();
    ...
    public TaskControlTowerBuilder UseTaskStateStore<T>() where T : class, ITaskStateStore
    {
    ...
    public TaskControlTowerBuilder UseTaskStateStore(Func<IServiceProvider, ITaskStateStore> factory)
    {
